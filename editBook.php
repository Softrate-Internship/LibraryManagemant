...
<?php
 ...
 $name = $_SESSION['name'];
 $pbid = mysqli_real_escape_string($conn,$_GET["bid"]);
 $sql = "SELECT * from books WHERE bid=$pbid";
...
<div style="padding: 10px 60px; border: 2px solid black;background-color: white;opacity: 0.9; width: fit-content;margin-left: 38%;margin-top: 12%;">
    <form method="post">
        <h1 style="text-align: center;font-size: 120%;">Make Changes</h1>
        <br>
        <label for="bid">Book ID: </label>
        <input type="number" name="bid" value="<?php echo $row['bid'];?>" style="margin-left: 5px;" required>
        <br><br><br>
        <label for="bname">Book Name: </label>
        <input type="text" name="bname" value="<?php echo $row['bname'];?>" style="margin-left: 5px;" required>
        <br><br><br>
        <label for="author"> Author: </label>
        <input type="text" name="author" value="<?php echo $row['author'];?>" style="margin-left: 5px;" required>
        <br><br><br>
...
<?php
    if($_SERVER["REQUEST_METHOD"]=="POST"){
        ...
        $author = $_POST['author'];
        $status = 1;
        $sql = "UPDATE books SET bid=$bid, bname='$bname', author='$author' WHERE bid = $pbid";
        $result = $conn->query($sql);
        if(!$result){?> 
        <script>
